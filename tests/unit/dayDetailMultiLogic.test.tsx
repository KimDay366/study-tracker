/**
 * 다중 로직 대응 회귀 테스트 (F6) — DayDetail
 * 로직 그룹마다 섹션이 반복 렌더링되는지(F3), 그리고 각 그룹의 카테고리 카드가
...
describe('DayDetail — 로직 그룹별 섹션 반복 렌더링 (F3 회귀 방지)', () => {
  it('그룹이 2개면 각 그룹의 로직명 헤더와 그 그룹 자신의 카테고리만 렌더링된다', () => {
    const groupA = makeGroup('logic-a', '오전 로직', '국어', 'cat-a', [makeSession('s1', 'cat-a', 30)]);
    const groupB = makeGroup('logic-b', '오후 로직', '영어', 'cat-b', [makeSession('s2', 'cat-b', 20)]);
    ...
    // 두 그룹의 로직명이 모두 섹션 헤더로 표시된다.
    expect(screen.getByText('오전 로직')).toBeDefined();
    expect(screen.getByText('오후 로직')).toBeDefined();

    // 각 그룹의 카테고리는 정확히 1개씩만 렌더링되고, 서로 섞이지 않는다.
  ...
  it('세션 내역 토글은 그룹별로 독립적으로 열리고 닫힌다(같은 카테고리 id라도 그룹이 다르면 섞이지 않음)', () => {
    // 두 그룹이 카테고리 id를 우연히 공유하는 극단적 케이스도 안전해야 한다.
    const groupA = makeGroup('logic-a', '로직A', '공통카테고리', 'cat-shared', [makeSession('s1', 'cat-shared', 10)]);
    const groupB = makeGroup('logic-b', '로직B', '공통카테고리', 'cat-shared', [makeSession('s2', 'cat-shared', 15)]);
