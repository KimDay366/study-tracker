/**
 * 다중 로직 대응 회귀 테스트 (F6) — useCalendarMonth
 *
 * §8-2 정책: 캘린더 달 뷰 하루 셀의 무지개 하트/별은 "하루 전체 합산"이 아니라
 * "로직 그룹별로 각각" 판정하고, 로직 수만큼 누적되어야 한다.
 * 이 테스트는 recordMap이 같은 날짜에 로직 그룹을 여러 개 담을 수 있게 된 뒤에도
...
function makeRecord(params: {
  ...
  const cat = { id: 'cat-1', name: '카테고리', colorVar: '--cat-color-01', targetMinutes: params.totalTargetMinutes, targetPercent: 100 };
  return {
    date: '2026-07-07',
...
describe('useCalendarMonth — 다중 로직 그룹별 집계 (§8-1, §8-2)', () => {
  it('같은 날짜에 로직 그룹이 2개면 groups 배열에 둘 다 담기고, 서로 독립적으로 달성률이 계산된다', () => {
    // 로직A: 30분 목표에 30분 채움 → 100%
    const groupA = makeRecord({
      logicId: 'logic-a', logicName: '로직A', totalTargetMinutes: 30,
      sessions: [makeSession('s1', 'cat-1', 30)],
    });
    // 로직B: 60분 목표에 10분만 채움 → 16.7%
    const groupB = makeRecord({
      logicId: 'logic-b', logicName: '로직B', totalTargetMinutes: 60,
    ...
    // 그룹 A는 100% 달성 → 하트·별 모두 획득 (카테고리 1개, 100% 이상)
  ...
  it('로직 그룹이 모두 100% 달성이면, group.rainbowHeart/rainbowStar(그룹 단위 필드)가 그룹 수만큼 true다', () => {
  ...
  it('무지개 별(day-level): 그날 로직이 여러 개이고 전부 달성이면 allLogicsAchieved는 true — 그룹 수와 무관하게 딱 1개짜리 판정', () => {
    const fullGroup = (logicId: string) => makeRecord({
      logicId, logicName: logicId, totalTargetMinutes: 10,
      sessions: [makeSession(`s-${logicId}`, 'cat-1', 10)],
    });
  ...
  it('무지개 별(day-level): 로직 중 하나라도 미달성이면 allLogicsAchieved는 false다', () => {
    const groupA = makeRecord({
  ...
  it('월간 달성률(monthStats.achievementPercent)은 그 달 모든 그룹을 각 100%로 캡한 뒤 평균한다(합산 방식 아님)', () => {
    // 목표 100분씩 3개 로직: 50%(50분) / 120%(120분, 초과) / 60%(60분) — 서로 다른 날짜에 걸쳐 있어도 월 전체로 합산돼야 한다.
